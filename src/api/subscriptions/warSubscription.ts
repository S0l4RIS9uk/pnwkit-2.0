 ...
 * @return {void}
 */
export default async function warSubscription(this: Kit, channel: string, event: string, callback: Function, cluster: subscriptionCluster = subscriptionCluster.OHIO): Promise<void> {
    ...
    newChannel.bind(`BULK_WAR_${event.toUpperCase()}`, callback);
