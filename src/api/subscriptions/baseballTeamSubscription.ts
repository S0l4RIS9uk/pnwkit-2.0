 ...
 * @return {void}
 */
export default async function baseballTeamSubscription(this: Kit, channel: string, event: string, callback: Function, cluster: subscriptionCluster = subscriptionCluster.OHIO): Promise<void> {
    ...
    newChannel.bind(`BULK_BBTEAM_${event.toUpperCase()}`, callback);
