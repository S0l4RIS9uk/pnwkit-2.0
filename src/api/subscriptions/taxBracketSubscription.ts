 ...
 * @return {void}
 */
export default async function taxBracketSubscription(this: Kit, channel: string, event: string, callback: Function, cluster: subscriptionCluster = subscriptionCluster.OHIO): Promise<void> {
    ...
    newChannel.bind(`BULK_TAX_BRACKET_${event.toUpperCase()}`, callback);
