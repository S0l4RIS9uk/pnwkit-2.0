 ...
 * @return {void}
 */
export default async function tradeSubscription(this: Kit, channel: string, event: string, callback: Function, cluster: subscriptionCluster = subscriptionCluster.OHIO): Promise<void> {
    ...
    newChannel.bind(`BULK_TRADE_${event.toUpperCase()}`, callback);
